...
function getDateList(_year, _month, checkedDay, isRange, checkedRange) {
  let list = [];
  ...
  while (isIterateContinue) {
    ...
    list[i] = {
      ...
      isStartDayChecked: isRange && !!checkedRange[0] && +checkedRange[0] - +newDate === 0,
      isEndDayChecked: !!checkedRange[1] && +checkedRange[1] - +newDate === 0,
      range: isRange && !!checkedRange[0] && !!checkedRange[1] && +newDate - +checkedRange[0] >= 0 && +checkedRange[1] - +newDate >= 0
    };
    i++;
    dates++;
...
function Calender(props) {
  ...
  const [checkedDay, setCheckedDay] = useState(_timeToDate(new Date()));
  ...
  const drawMonth = () => {
    ...
    setMonth(m2);
    for (let i = 0; i < 3; i++) {
      list[i] = getDateList(y, m2 - 2 + i, checkedDay, isRange, checkedRange);
    }
    setListData(list);
    setTransitionDuration(0);
    setTranslateX(-calenderBoxWidth);
    if (isRange) {
      if (((_a = checkedRange[0]) == null ? void 0 : _a.getFullYear()) === weekDay.getFullYear() && checkedRange[0].getMonth() === weekDay.getMonth()) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        childList[i] = {
          ...
          isStartDayChecked: isRange && !!checkedRange[0] && +checkedRange[0] - +d === 0,
          isEndDayChecked: !!checkedRange[1] && +checkedRange[1] - +d === 0,
          range: isRange && !!checkedRange[0] && !!checkedRange[1] && +d - +checkedRange[0] >= 0 && +checkedRange[1] - +d >= 0
        };
      }
      list[j] = childList;
    }
  ...
  const selectRange = (startDate, endDate, list) => {
    const newList = [...list];
  ...
  useEffect(() => {
    ...
    if (isRange) {
      if (defaultCheckedRange && defaultCheckedRange[0] && defaultCheckedRange[1] && _verifyDate(defaultCheckedRange[0]) && _verifyDate(defaultCheckedRange[1]) && +defaultCheckedRange[0] - +defaultCheckedRange[1] < 0) {
        setCheckedRange([_timeToDate(defaultCheckedRange[0]), _timeToDate(defaultCheckedRange[1])]);
